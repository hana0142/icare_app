    ...
    getResult: async (req, res) => {
        try {
            if ((req.params.user_id) && (req.params.page_no) && (req.params.limit)) {
                ...
                //결과 조회
                const all_result = await ResultService.ResultList.selectCheckInfo(user_id, page_no, limit);
        ...
        try {
            if (req.params.check_id) {
                ...
                if (check_category === 'vs') {
                    const vs_result = await ResultService.VisionCheck.selectByCheckID(check_id);
                ...
                //암점자가인식검사
                    ...
                    //암점자가인식검사 전체 내역
                    const bs_result = await ResultService.BlindSpotCheck.selectByCheckId(check_id);
                    //암점자가인식 좌안검사 결과
    ...
    getResult: async (req, res) => {
        try {
            if (req.params.user_id) {
                const user_id = req.params.user_id;
                const vs_result = await ResultService.VisionCheck.selectByUserId(user_id);
    ...
    getResultDetail: async (req, res) => {
        try {
            if (req.body.check_vs_no) {
                ...
                const vs_detail_result = await ResultService.VisionCheck.selectResultCheckId(check_vs_no);
    ...
    getResult: async (req, res) => {
        try {
            if ((req.params.check_id) && (req.params.user_id)) {
                const user_id = req.params.user_id;
                let bs_result = await ResultService.BlindSpotCheck.selectAllResult(user_id);
...
exports.EyeMovementCheck = {
    getResult: async (req, res) => {
        try {
            if (req.cookies.user_id) {
                const user_id = req.cookies.user_id;
                let em_result = await ResultService.EyeMovementCheck.selectAllResult(user_id);
